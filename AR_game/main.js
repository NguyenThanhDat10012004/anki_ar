...
        
        // ⬇️ THAY ĐỔI 1: ĐÃ XÓA BỎ mindarThree.stop()
        // Chúng ta không dừng engine, chỉ giấu nó đi bằng CSS
...
const showGameScreen = () => { // Bỏ 'async'
    showScreen(null); // Chỉ cần lộ AR canvas ra
    
    // ⬇️ THAY ĐỔI 2: ĐÃ XÓA BỎ mindarThree.start()
    // Engine đã chạy rồi, không cần khởi động lại
...
        if (animal.scene) {
            animal.scene.visible = true; // Bật hiển thị các con vật
...
      imageTargetSrc: './targets.mind', // Đảm bảo bạn dùng file .mind ổn định
    });
...
        animal.scene.visible = false; // QUAN TRỌNG: Ẩn chúng đi lúc đầu
...
    });
    ...
    // ⬇️ THAY ĐỔI 3: Khởi động AR 1 LẦN DUY NHẤT
    // Engine sẽ chạy ngầm ngay cả khi đang ở màn hình menu
    await mindarThree.start();

    // BẮT ĐẦU VỚI MÀN HÌNH KHỞI ĐỘNG (AR lúc này đã chạy ngầm)
    showStartScreen();
  }
